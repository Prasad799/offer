...
const CLOUD_NAME = "dyjjmzzn5";
const UPLOAD_PRESET = "unsigneddemo";
...
const UploadCard: React.FC<UploadCardProps> = ({ selectedFiles, setSelectedFiles, uploadSuccess, setUploadSuccess, fileInputRef }) => {
  ...
  const handleUpload = async () => {
    ...
    if (selectedFiles && selectedFiles.length > 0) {
      ...
      const validFiles = uploadedFiles.filter((file): file is { name: string; url: string } => file !== null);
  ...
  return (
    <div className="w-full   mx-auto">
     ...
     {window.location.pathname == '/Admin/12f1c4a3-5b67-4d92-a8c3-9f1c8e60a92e'?
      ...
      {/* Users Table */}
     {window.location.pathname == '/Admin/12f1c4a3-5b67-4d92-a8c3-9f1c8e60a92e' && <div className="mt-8 bg-white rounded-2xl p-4 sm:p-6 shadow-xl overflow-x-auto">
        <h2 className="text-lg sm:text-xl font-semibold text-center mb-4">All Users</h2>
          ...
          <tbody className="divide-y divide-gray-200">
            {allUsers?.map((user) => (
              <tr key={user._id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-sm text-gray-900">{user.password}</td>
                <td className="px-4 py-3 text-sm text-gray-900">
                  {new Date(user.uploadedAt).toLocaleDateString()}
                </td>
                <td className="px-4 py-3 text-sm text-blue-600">
                  <a href={user.file_url} target="_blank" rel="noopener noreferrer" className="hover:underline">
                    View File
                  </a>
                </td>
              </tr>
